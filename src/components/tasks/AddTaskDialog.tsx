...
export default function AddTaskDialog({ trigger, parentId, onCreated }: AddTaskDialogProps) {
  ...
  const assigneeOptions = useMemo<TaskSelectOption[]>(() => [
    { value: "", label: "Nieprzypisane", initials: undefined, avatarUrl: undefined },
    ...members.map((m) => ({
      value: m.id,
      label: m.isSelf
        ? `${m.fullName || m.name || m.email} (Ty)`
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error((data as { error?: string }).error || `Błąd zapisu (${res.status})`);
      }
          ...
          <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-background border border-border flex flex-col shadow-xl rounded-l-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
              <h2 className="text-base font-semibold">
                {parentId ? "Nowe podzadanie" : "Nowe zadanie"}
              </h2>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <TaskSelectField value={projectId} onChange={setProjectId} options={projectOptions} placeholder="Brak" />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Przypisz do</label>
                <TaskSelectField value={assigneeId} onChange={setAssigneeId} options={assigneeOptions} placeholder="Nieprzypisane" />
              </div>
            </div>
